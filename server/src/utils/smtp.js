...
class SMTP {
    static async sendEmail(email, subject, message, callback) {
        try {
            ...
            console.error(`⛔ Error sending ${callback} to ${email}`, error);
        }
    }
    ...
    static async sendRecovery(email, tokenRecovery) {
        ...
        const message = `<a href=http://localhost:3000/auth/pass-create?tokenRecovery=${tokenRecovery}>Click here to create a new password</a>`
